...
const fetchData = async (collectionName, setState) => {
  switch (collectionName) {
    case "customSchedules":
      return await fetchCustomSchedules(setState);
    case "inactiveSchedules":
      return await fetchInactiveSchedules(setState);
    case "vacaciones":
      return await fetchVacaciones(setState);
    default:
      console.warn(`No se reconoce la colección: ${collectionName}`);
      break;
  }
};

const getCurrentState = async (collectionName) => {
  const snapshot = await getDocs(collection(db, collectionName));
  return snapshot.docs.map(doc => doc.data());
};

const saveData = async (collectionName, data, setState, closeModal) => {
  try {
    const currentState = await getCurrentState(collectionName);
    switch (collectionName) {
      case "customSchedules":
        return await handleSaveCustomSchedule(data, setState, currentState, closeModal);
      case "inactiveSchedules":
        return await handleSaveInactiveSchedule(data, setState, currentState, closeModal);
      case "vacaciones":
        return await handleSaveVacaciones(data, setState, currentState, closeModal);
      default:
        console.warn(`No se reconoce la colección: ${collectionName}`);
        break;
    }
